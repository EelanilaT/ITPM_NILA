...
export default function AllJobss() {
    ...
    const getData = async () => {
        await axios.get("http://localhost:5001/jobs")
        .then((res) => {            
            setJobs(res.data)
        })
        .catch((err) => {
    ...
    if(!jobs)
    return alert("No Jobs")

     const onUpdate = (data) => {
        localStorage.setItem('id', data._id);
        localStorage.setItem('title', data.title);
        localStorage.setItem('date', data.date);
        localStorage.setItem('date', data.location);
        localStorage.setItem('workingmode', data.workingmode);
        localStorage.setItem('requirement', data.requirement);
    ...
    return(
        <div className="container">
            <h1 style={{textAlign: 'center', fontFamily: 'cursive', fontSize: '50px'}}>All Job Details</h1>
            <div className="container">
                <div>
                    ...
                    <tbody >
                        {filteredJobs.map(job => (
                            <tr key={job._id}>
                                ...
                                <td style={{ textAlign: 'center' }}>{new Date(job.date).toISOString().slice(0,10)}</td>
                                ...
                                <td>    
                                    <button type='button' className="btn-update" onClick={() => onUpdate(job)}>Update</button>
                                    &nbsp;
                                    <button type='button' className="btn-delete" onClick={() => onDelete(job._id)}>Delete</button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
